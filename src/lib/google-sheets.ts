...
const MEMBERS_HEADERS = ["email", "name", "avatarUrl", "createdAt", "maxPoints"]; // Added maxPoints
...
export async function ensureMember(user: { email: string; name: string; image?: string; maxPoints?: number }) {
    const rows = await getSheetData(MEMBERS_TAB);
    ...
    if (rowIndex === -1) {
        // New member - append row with initial name from Google account and maxPoints
        await appendRow(MEMBERS_TAB, [
            user.email,
            user.name,
            user.image || "",
            Date.now().toString(),
            (user.maxPoints || 15).toString() // Save maxPoints
        ]);
    } else {
        // Existing member - update avatarUrl (column C) and maxPoints (column E)
        const sheetRow = rowIndex + 1; // 1-based for Sheets API

        // Update avatarUrl (Col C)
        ...
        // Update maxPoints (Col E) if provided. Note: Col D is createdAt, E is maxPoints.
        // If we want to update E without touching D, we can reference E directly?
        // Wait, range C is only 1 cell.
        // We can do separate update for E.
        if (user.maxPoints) {
            await sheets.spreadsheets.values.update({
                spreadsheetId: SHEET_ID,
                ...
                valueInputOption: "RAW",
                requestBody: {
                    values: [[user.maxPoints.toString()]]
                },
            });
        }
    }
...
export async function getTasks(filterEmail?: string): Promise<Task[]> {
    ...
    const allTasks = rows.slice(1).map((row) => ({
        ...
        assignedTo: row[11] || undefined, // Column L
        order: Number(row[12]) || 0, // Column M
    })) as Task[];
...
export async function addTask(task: Task) {
    const row = [
        ...
        task.assignedTo || "", // Column L
        task.order?.toString() || "0", // Column M
    ];
...
export async function updateTask(id: string, updates: { content?: string; weight?: number; category?: string; notes?: string; priority?: string; repeatWeekly?: boolean; assignedTo?: string; order?: number }) {
    const rows = await getSheetData(TAB_TITLE);
    ...
    const newAssignedTo = updates.assignedTo ?? currentRow[11] ?? "";
    const newOrder = updates.order?.toString() ?? currentRow[12] ?? "0";

    const currentWorkWeek = currentRow[7]; // Get existing workWeek

    // Update columns columns C-D (content, weight)
    await sheets.spreadsheets.values.update({
        spreadsheetId: SHEET_ID,
    ...
    // Update columns G, H, I, J, K, L, M (category, workWeek, notes, priority, repeatWeekly, assignedTo, order)
    // NOTE: We must preserve H (workWeek)
    await sheets.spreadsheets.values.update({
        spreadsheetId: SHEET_ID,
        range: `${TAB_TITLE}!G${sheetRow}:M${sheetRow}`,
        valueInputOption: "RAW",
        requestBody: { values: [[newCategory, currentWorkWeek, newNotes, newPriority, newRepeatWeekly, newAssignedTo, newOrder]] },
    });
}
